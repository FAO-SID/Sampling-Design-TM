...
```{r PCA_visible_06, eval=FALSE, include=TRUE, message=FALSE, warning=FALSE}
  ...
  cov.dat <- pca$PCA
  # Subset rasters
  n_comps <- first(which(pca$summaryPCA[3,]>0.99)) # Select PC explaining >=99%
  cov.dat <- pca$PCA[[1:n_comps]]
...
```{r clhs-legacy-samples, eval=TRUE, include=TRUE,warning=FALSE, message=FALSE}
...
```{r clhs-buffer-06, eval=TRUE, include=TRUE}
...
```{r clhs-reclass-06, eval=TRUE, include=TRUE, warning=FALSE}
...
```{r clhs-polygonize-06, eval=TRUE, include=TRUE}
...
```{r clhs-export-06, eval=FALSE, include=TRUE, warning=FALSE, results='hide'}
