...
# Load packages as a vector objects
  packages <- c("sp", "terra", "raster","clhs", "entropy", "tripack","manipulate","dplyr","plotly") # Create a vector of packages to use
...
```{r setup_wd, eval=TRUE, include=TRUE}
...
```{r load_covariates, eval=TRUE, include=TRUE, warning=FALSE}
  ...
  # Load soil data
    p.dat <- terra::vect("clhs_data/shapes/legacy_soils.shp")
  ...
  ## Load raster covariate data
    ...
    #  Artificial dissagregate to build layer statistics
    cov.dat <- disagg(cov.dat, fact=1, method="bilinear")
    # Simplify data rasters for calculations 
    cov.dat <- aggregate(cov.dat, fact=5, fun="mean") # Now you can see the statistics
...
This indicates that `r round(sum(newScores$pip)/nrow(newScores)*100,1)`\% of the existing conditions in the study area fall within the convex hull delineated with the data in the  soil samples, showing the level of adequacy of the proposed legacy data for DSM in the area.
