...
# Set working directory to source file location
  setwd("/Users/luislado/Dropbox/Github/soil_sampling")
...
```{r plot_covdata, eval=TRUE, include=TRUE, fig.cap="Plot of the covariates"}
   plot(cov.dat)
...
```{r legacy_data_adequacy, eval=T, include=TRUE}
...
This indicates that `r round(sum(newScores$pip)/nrow(newScores)*100,1)`\% of the existing conditions in the study area fall within the convex hull delineated with the data in the  soil samples, showing the adecquacy of the proposed legacy data for DSM.
