...
The target points are obtained using the `'cLHS'`function together with the stack of raster covariates and the number of samples. The function uses a number of iterations for the Metropolis-Hastings annealing process, with a default of 10000. We sample the covariate data set and pick a subset of points equal to the sample size determined previoulsly. 

```{r fig-cLHS_design_05, fig.cap="Evolution of the objective function", eval=TRUE, include=TRUE}

  pts <- clhs(cov.dat, size = optimal_n, iter = 5000, progress = FALSE, simple = FALSE)
  plot(pts, c('obj'))
```
...
In situations where there are legacy soil data samples available, it would be interesting to include them in the cLHS design to increase the diversity of covriates and avoid oversampling for some conditions. In this cases, the ancillary data can be included in the design as additional points to the clhs fuction.
...
# We create an artificial legacy dataset of 20 samples over the study area
  legacy.data <- spatSample(cov.dat,10, na.rm=TRUE,xy=TRUE,method="random")
# Get covariates data as a dataframe with coordinates
  cov.df <- cbind(crds(cov.dat),as.data.frame(cov.dat))
...
# Calculate clhs points with legacy data
    res <- clhs(x = leg.new, size = 30,  iter = 5000,simple = FALSE, progress = FALSE,
...
Figure \@ref(fig:fig-16) shows the distribution of new samples along with legacy data points.

```{r fig-16, fig.cap="cLHS sampling points with legacy data", eval=TRUE, include=TRUE}

# Plot points
  plot(cov.dat[[1]], main="Samples (point) and legacy samples (cross)")
  points(points, col="dodgerblue", pch = 16)
  points(legacy.data, col="red", pch = 3)

```

## Workin with large raster data

The cLHS function samples the raster stack to analyse the structure of covariate data in order to determine the optimal location os samples that represent the environmental conditions in the area. In the case of working with large raster sets, the process can be higly computing demanding since all pixels in the raster stack is used in the process. A means to avoid this is to sample  the stack on a regular grid to account for a shorter number of data points and use this information as input within the cLHS function. 

The creation of a regular point grid on the raster stack is straightforward through the function `spatSample` from the `'terra'` package.


```{r largedata01-05, fig.cap="cLHS sampling points with legacy data", eval=TRUE, include=TRUE}
# result is a SpatialPointsDataFrame
# size is the number of points to generate

regular.sample <- spatSample(cov.dat, size = 500, xy=TRUE, method="regular")
regular.sample<- na.omit(regular.sample)
# plot s to see the grid of points
plot(cov.dat[[1]], main="Regular sampling raster data)")
points(regular.sample)

```


```{r largedata02-05, eval=TRUE, include=TRUE}

regular.sample.clhs <- clhs(regular.sample, size = 100, progress = FALSE, iter = 1000, simple = FALSE)
plot(regular.sample.clhs)
plot(regular.sample.clhs, mode=c("obj", "box"))
...
CLHS can also be implemented taking into account environmental features such as terrain (elevation, slope) or accessibility (distance to roads) that can alter the sampling process. This is done by defining constraints as 'cost' layers in the options of the `'clhs'` function.

```{r fig-18, fig.cap="cLHS sampling with cost layers", eval=TRUE, include=TRUE}
  cov.dat <- raster::stack(cov.dat)
  ## Create a cLHS sampling point set----
  pts <- clhs(cov.dat, size = optimal_n, iter = 5000, progress = FALSE, simple = FALSE, cost = 'elevation',  use.cpp = F)
  plot(pts, c('obj', 'cost'))
```
...
The similarity probabilities for the first cLHS point is presented on Figure \@ref(fig:fig-19) over the elevation layer. 

```{r fig-19, fig.cap="Probability of similarity in the buffer for the first cLHS point (in black) over elevation. The blue crosses represent the location of the remaining cLHS points from the analysis.", eval=TRUE, include=TRUE}
...
The results are shown in Figure \@ref(fig:fig-20).

```{r fig-20, fig.cap="Distribution of cLHS sampling points in the study area", eval=TRUE, include=TRUE}
...
```{r cLHS_export_05, eval=FALSE, include=TRUE, warning=FALSE}
    
   ## Write replacement areas to polygon shape 
    s <- st_as_sf(s)
    st_write(s, paste0('replacement_areas_', D, '.shp'), delete_dsn = TRUE)
   
    ## Write  cLHS sampling points to shapefile
    pts <- st_as_sf(pts$sampled_data)
    st_write(pts, 'target_clhs.shp', delete_dsn = TRUE)
