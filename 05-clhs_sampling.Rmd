...
```{r fig-22b, fig.cap="cLHS sampling with legacy data, cost surface and distance buffers around roads", eval=TRUE, include=TRUE, warning=FALSE}

  roads <- sf::st_read(file.path(paste0(shp.path,"/road.shp")))
  legacy <- sf::st_read(file.path(paste0(shp.path,"/legacy_soils.shp")))
  cost <- calculate_distance(raster = cov.dat, access = roads)
  cost$geology<-cost$geology*cost$claymin/cost$claymin
  cost$dist2access<-cost$dist2access*cost$claymin/cost$claymin
  plot(cost$dist2access)
  plot(roads,add=T)
  aa <- sample_clhs( mraster = cost, nSamp = 300, existing = legacy, iter = 250, details = TRUE, cost="cost", access=roads, buff_inner=20, buff_outer=300)
  plot(cost$dist2access)
...
Legacy data is represented as blue dots while new samples from cLHS analyses are in red color (Fig.\@ref(fig:fig-22)). Note that the new sampling points are located within a distance buffer of 20-300 meters from roads. In addition, a cost surface has also been included in the analyses.
