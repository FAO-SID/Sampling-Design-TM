...
The similatity probabilities for the fisrt cLHS point is presented on Figure \@ref(fig:fig-16).over the elevation layer. 

```{r fig-16, fig.cap="Similarity Probability in the buffer for the first cLHS point over elevation", eval=TRUE, include=TRUE}
...
The reclassified raster stack is then converted to an object of `'SpatialPolygonsDataFrame'` class that can be exported to a shapefile to be used in any GIS.
