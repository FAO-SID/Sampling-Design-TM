...
for (dede in seq(initial.n, final.n, by=by.n)){
 for (kaka in 1:10){
  ...
  samples_storage[[paste0("N", dede,"_",kaka)]] <- p.dat_I_sp
  ...
  #print(paste("N samples = ",dede, " out of ",final.n, " ;kaka = ",kaka," ;KL = ", klo, ";Proportion = ", sep sum(newScores$pip)/nrow(newScores)*100 ))
  }
  }
...
```{r results-table-03, eval=TRUE, include=TRUE}
knitr::kable(
...
plot_ly(mean_result,
  ...
  layout(xaxis = list(title = "N", 
         ...
         yaxis2 = list(title = "mean Representativeness (%)",
...
plot(cov.dat[[1]])
N_final <- samples_storage[paste0("N",optimal_n$N,"_",optimal_pass$IDX)][[1]]
points(N_final)
