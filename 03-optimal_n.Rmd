...
```{r fig-7a, fig.cap="Boxplot of the dispersion in KLO and % repressentativeness in the iteration trials for each sample size",fig.width=8, fig.height=5, eval=TRUE, include=TRUE}
...
## Determine the optimal iteration according to the optimal N size 
  optimal_iteration <- results[which(abs(results$N - optimal_n) == min(abs(results$N - optimal_n))),] %>%
    mutate(IDX = 1:n()) %>%
