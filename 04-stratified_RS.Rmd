...
```{r regular-sampling, eval=TRUE, include=FALSE}
      # Stratified Simple regular Sampling over Soil/Land use Strata -------------------------------------------
...
```{r fig:regular-sampling-plot, eval=TRUE, include=TRUE, warning=FALSE, fig.cap="Plot of strata and sampling points"}

  soil_lc <- st_read("../soil_sampling/JAM/strata_diss.shp")
  soil_lc <- st_cast(soil_lc, 'POLYGON')
  target <- st_read("../soil_sampling/JAM/grid_sampling_points.shp")
  target <- target[target$type=="Target",]
  # Plot final map with the aggregated land use information
  #mapview(soil_lc["soil_lc"], alpha=0, homebutton=T) + mapview(z[z$type=="Target",], color = "white", col.regions = "tomato")
  terra::plot(soil_lc["soil_lc"], border=NA, main="Strata classes")
  #terra::points(target[target$type=="Target","type"], pch=16, col="black")
