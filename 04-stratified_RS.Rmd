...
# Load packages as a vector objects
  packages <- c("sf", "terra", "tidyverse", "rmapshaper", "units","plyr", "mapview")
...
```{r strata-parameters, eval=TRUE, include=FALSE, warning=FALSE}
...
```{r soil-aggregated-plot, eval=TRUE, include=TRUE, fig.cap="Plot of the soil classes"}

  # Plot final map with the aggregated soil information
  #mapview(soil["USDA_CLASS"], alpha=0, homebutton=T)
...
```{r landuse-aggregated-plot, eval=TRUE, include=TRUE, fig.cap="Plot of the land use classes"}

  # Plot final map with the aggregated land use information
  #mapview(lc["LU"], alpha=0, homebutton=T)
...
```{r combine-data, eval=TRUE, include=TRUE} 
...
The final strata map is shown in Figure XX.

```{r strata-plot, eval=TRUE, include=TRUE, fig.cap="Plot of strata"}
...
# Stratified Simple Random Sampling over Soil/Land use Strata -------------------------------------------
  ...
  # Clean memory
  rm(list=setdiff(ls(), c("r","n","distance.buffer","REPLACEMENT", "samples.buffer"))) 
  
  # Read in shapefile containing polygons and their areas
...
# Generate random points within each strata of size 3 tmes the required samples for each strata ----
...
# Export data to Shapefile ----
  ...
  writeVector(z, "../soil_sampling/JAM/sampling_points.shp", overwrite=TRUE)
  
  # Write sampling data as csv
  z <- st_as_sf(z) %>% st_transform(crs = 4326)
  z_df <- z %>% 
    st_coordinates() %>% 
    cbind(z) %>% 
    st_drop_geometry() %>% 
    dplyr::select(ID, type, code, order, X, Y, group)
  write_csv(z_df, "../soil_sampling/JAM/sampling_points.csv")
  
  legend <- z_df %>% 
    dplyr::select(code, group) %>% 
    unique()
  legend <- arrange(legend, as.numeric(code))
  write_csv(legend, "../soil_sampling/JAM/legend_JAM.csv")  
...
```{r fig:sampling-plot, eval=TRUE, include=TRUE, warning=FALSE, fig.cap="Plot of strata and sampling points"}
  ...
  # Plot final map with the aggregated land use information
  #mapview(soil_lc["soil_lc"], alpha=0, homebutton=T) + mapview(z[z$type=="Target",], color = "white", col.regions = "tomato")
  terra::plot(soil_lc["soil_lc"], border=NA, main="Strata classes")
