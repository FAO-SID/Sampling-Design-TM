...
```{r fig-10, fig.cap="Plot of the soil classes", eval=FALSE, include=FALSE}
  
#The soil classes used to build the strata are shown in Figure \@ref(fig:fig-10).
...
```{r fig-11, fig.cap="Plot of the land use classes", eval=FALSE, include=FALSE}
  ...
  mv <- mapview(lc["LU"], alpha=0, homebutton=T, layer.name = "Landuse")
  mv@map
```
...
# Stratified Simple Random Sampling over Soil/Land use Strata -------------------------------------------
  ...
  # Clean memory
  #rm(list=setdiff(ls(), c("r","n","distance.buffer","REPLACEMENT", "samples.buffer", "soil_lc","optimal_n","cov.dat"))) 
...
```{r fig-13, fig.cap="Plot of strata and random, sampling points", eval=TRUE, include=TRUE}

  mv <- mapview(soil_lc["soil_lc"], alpha=0, homebutton=T, layer.name = "Strata") +
  mapview(z[z$type=="Target",], color = "white", cex=3, col.regions = "tomato",layer.name = "Target Points")
  mv@map
...
```{r fig-14, fig.cap="Plot of strata and regular sampling points", eval=TRUE, include=TRUE}
  ...
  mv <- mapview(soil_lc["soil_lc"], alpha=0, homebutton=T, layer.name = "Strata") + mapview(z[z$type=="Target",], cex=3, color = "white", col.regions = "tomato",layer.name = "Target Points")
  mv@map
```
