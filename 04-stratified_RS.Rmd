...
# Load packages as a vector objects
  packages <- c("sf", "terra", "tidyverse", "rmapshaper", "units","plyr", "mapview", "leaflet")
...
```{r fig-10, fig.cap="Plot of the soil classes", eval=FALSE, include=TRUE}
...
```{r fig-11, fig.cap="Plot of the land use classes", eval=FALSE, include=TRUE}
...
```{r fig-13, fig.cap="Plot of strata and random target and replacement points", eval=TRUE, include=TRUE}
  ...
  mv <- mapview(soil_lc["soil_lc"], alpha=0, homebutton=T, layer.name = "Strata") +
    mapview(z[z$type=="Target",], color = "white", cex=3, col.regions = "dodgerblue3",layer.name = "Replacement Points") +
    mapview(z[z$type=="Target",], color = "white", cex=3, col.regions = "tomato",layer.name = "Target Points", map=map)
  mv@map
